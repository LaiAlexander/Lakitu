...
class CogManager(commands.Cog):
        ...
        if cog:
            async with ctx.typing():
                ...
                try:
                    ...
                    embed.add_field(name=f"Loaded {cog}!", value="Success!", inline=False)
                    print(f"{cog} cog has been loaded\n-----", flush=True)
                except Exception as e:
                    embed.add_field(name=f"Failed to load {cog}.", value=e, inline=False)
                    print(f"Execption: {e}", flush=True)
        else:
            async with ctx.typing():
    ...
    async def unload_cog(self, ctx, cog=None):
        ...
        if cog == this_cog:
            print("Don't ever unload cog_manager", flush=True)
            embed = discord.Embed(title=f"Unloading {cog}")
            embed.add_field(name=f"Not allowed to unload {cog}", value="Denied", inline=False)
            await ctx.send(embed=embed)
            return
        if cog:
            async with ctx.typing():
                ...
                try:
                    ...
                    embed.add_field(name=f"Unloaded {cog}!", value="Success!", inline=False)
                    print(f"{cog} cog has been unloaded\n-----", flush=True)
                except Exception as e:
                    embed.add_field(name=f"Failed to unload {cog}.", value=e, inline=False)
                    print(f"Execption: {e}", flush=True)
        else:
            async with ctx.typing():
